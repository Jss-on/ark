...
int check_system_activity(void) {
    ...
    if (get_cpu_usage(&cpu_percent) == 0) {
        if (cpu_percent > g_config.cpu_threshold) {
            if (current_time - g_last_detailed_log >= 30) {
                snprintf(log_buffer, sizeof(log_buffer), "CPU activity: %.2f%% (threshold: %.2f%%)", 
                         cpu_percent, g_config.cpu_threshold);
...
void parse_args(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "c:w:m:i:p:e:n:l:dh", long_options, &option_index)) != -1) {
        switch (opt) {
            case 'c':
